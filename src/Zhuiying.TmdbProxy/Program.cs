...
app.MapGet("/api/search", async (string q, IHttpClientFactory f, [AsParameters] QueryParams qp) =>
{
    ...
    foreach (var i in mj.RootElement.GetProperty("results").EnumerateArray()) { var d = JsonHelper.ToDict(i); d["media_type"] = "movie"; results.Add(d); }
    foreach (var i in tj.RootElement.GetProperty("results").EnumerateArray()) { var d = JsonHelper.ToDict(i); d["media_type"] = "tv"; results.Add(d); }
    results.Sort((a, b) =>
    {
        var da = a.GetValueOrDefault("release_date")?.ToString() ?? a.GetValueOrDefault("first_air_date")?.ToString();
        var db2 = b.GetValueOrDefault("release_date")?.ToString() ?? b.GetValueOrDefault("first_air_date")?.ToString();
        return string.IsNullOrEmpty(da) ? 1 : string.IsNullOrEmpty(db2) ? -1 : string.Compare(db2, da, StringComparison.Ordinal);
    });
